Feature: Server and Dashboard configuration generation
  ...
  Scenario: Server config contains internal database URLs
    Given an AxonOpsServer CR with internal TimeSeries and Search
    When the operator reconciles the AxonOpsServer CR
  ...
  Scenario: Server config contains external database URLs
    Given an AxonOpsServer CR with external TimeSeries hosts "cassandra-1.example.com,cassandra-2.example.com"
  ...
  Scenario: Server config includes TLS settings for internal databases
    Given an AxonOpsServer CR with internal TimeSeries and Search with TLS enabled
    When the operator reconciles the AxonOpsServer CR
  ...
  Scenario: Dashboard config does not enable SSL
    Given an AxonOpsServer CR with Dashboard enabled
    When the operator reconciles the AxonOpsServer CR
