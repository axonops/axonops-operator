Feature: AxonOpsPlatform components
  ...
  Scenario: Custom resource requests and limits for Dashboard
    Given an AxonOpsPlatform CR with Dashboard resources:
      | resource | requests | limits |
      | cpu      | 100m     | 500m   |
      | memory   | 128Mi    | 512Mi  |
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Custom heap size for TimeSeries
    Given an AxonOpsPlatform CR with TimeSeries heapSize "2048M"
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Custom heap size for Search
    Given an AxonOpsPlatform CR with Search heapSize "4g"
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Default heap sizes when not specified
    Given an AxonOpsPlatform CR with no heap size overrides
    When the operator reconciles the AxonOpsPlatform CR
