Feature: Support external SearchDB (OpenSearch/Elasticsearch)
  As an operator user
  I want to configure AxonOpsServer to use an external OpenSearch or Elasticsearch cluster for SearchDB
  ...
  Scenario: Deploy with external OpenSearch (no internal StatefulSet)
    Given an AxonOpsServer CR with spec.search.enabled set to true
    ...
    And the AxonOpsServer status should reflect "Search: External"

  Scenario: Deploy with external Elasticsearch
    Given an AxonOpsServer CR with spec.search.external.hosts set to ["https://elasticsearch.prod.internal:9200"]
  ...
  Scenario: External SearchDB with TLS enabled
    Given an AxonOpsServer CR with spec.search.external.hosts set to ["https://search.prod.internal:9200"]
  ...
  Scenario: External SearchDB with TLS skip verify (self-signed certs)
    Given an AxonOpsServer CR with spec.search.external.hosts configured
    And spec.search.external.tls.enabled set to true
  ...
  Scenario: Switch from internal to external SearchDB
    Given an existing AxonOpsServer with an internal axondb-search StatefulSet running
  ...
  Scenario: External SearchDB becomes unreachable
    Given an AxonOpsServer CR configured with external Search hosts
  ...
  Scenario: Both TimeSeries and Search external
    Given an AxonOpsServer CR with both external TimeSeries and external Search configured
    ...
    And the AxonOpsServer status should reflect "TimeSeries: External, Search: External"
