Feature: Pod customization with annotations, labels, environment variables, and volumes
  ...
  Scenario: Custom annotations on component pods
    Given an AxonOpsPlatform CR with custom annotations on the Server component:
      | key                              | value   |
  ...
  Scenario: Custom labels on component pods
    Given an AxonOpsPlatform CR with custom labels on the Dashboard component:
      | key         | value      |
      | team        | platform   |
  ...
  Scenario: Custom environment variables on Dashboard
    Given an AxonOpsPlatform CR with Dashboard environment variables:
      | name          | value              |
  ...
  Scenario: Extra volumes and volume mounts on Server
    Given an AxonOpsPlatform CR with extra volumes on the Server component:
  ...
  Scenario: Extra volumes and volume mounts on Dashboard
    Given an AxonOpsPlatform CR with extra volumes on the Dashboard component:
      | volume_name | type   | source          |
