Feature: Dashboard replica configuration
  ...
  Scenario: Dashboard uses default replica count of 1
    Given an AxonOpsServer CR with Dashboard enabled and no replica override
  ...
  Scenario: Dashboard with custom replica count
    Given an AxonOpsServer CR with Dashboard replicas set to 3
    When the operator reconciles the AxonOpsServer CR
    Then the Dashboard Deployment should have 3 replicas

  Scenario: Update Dashboard replica count
    Given an AxonOpsServer CR with Dashboard replicas set to 1
