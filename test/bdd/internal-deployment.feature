Feature: AxonOpsPlatform components with internal databases
  As a platform operator
  I want to deploy a complete AxonOpsPlatform stack with all four components (TimeSeries, Search, Server, Dashboard)
  ...
  Scenario: All components have correct owner references
    Given an AxonOpsPlatform CR named "axon-prod" with all components enabled
    When the operator reconciles the AxonOpsPlatform CR
    Then the axondb-timeseries StatefulSet should be owned by the AxonOpsPlatform CR
    And the axondb-search StatefulSet should be owned by the AxonOpsPlatform CR
    And the axon-server StatefulSet should be owned by the AxonOpsPlatform CR
    And the axon-dash Deployment should be owned by the AxonOpsPlatform CR
    And all Services and Secrets should be owned by the AxonOpsPlatform CR

  Scenario: Status conditions reflect internal component mode
    Given an AxonOpsPlatform CR with all components configured for internal deployment
    When the operator reconciles the AxonOpsPlatform CR
    Then the AxonOpsPlatform status should have condition "CertManagerReady=True"
  ...
  Scenario: Headless Services created for StatefulSet DNS
    Given an AxonOpsPlatform CR with axondb-timeseries and axondb-search enabled
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: ClusterIP Services created alongside headless Services
    Given an AxonOpsPlatform CR with all internal database components enabled
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Server config Secret contains correct database URLs
    Given an AxonOpsPlatform CR named "axon-prod" with internal TimeSeries and Search enabled
