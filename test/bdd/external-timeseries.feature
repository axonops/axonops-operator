Feature: Support external TimeSeries (Cassandra) database
  As an operator user, I want to configure AxonOpsPlatform to use an external Cassandra cluster
  ...
  Scenario: Deploy with external Cassandra (no internal StatefulSet)
    Given an AxonOpsPlatform CR with spec.timeSeries.enabled set to true
    ...
    And the AxonOpsPlatform status should reflect "TimeSeries: External"

  Scenario: External Cassandra with TLS enabled
    Given an AxonOpsPlatform CR with spec.timeSeries.external.hosts set to ["cassandra.prod.internal:9042"]
  ...
  Scenario: Switch from internal to external TimeSeries
    Given an existing AxonOpsPlatform with an internal axondb-timeseries StatefulSet running
  ...
  Scenario: External Cassandra becomes unreachable
    Given an AxonOpsPlatform CR configured with external TimeSeries hosts
  ...
  Scenario: Missing authentication for external Cassandra
    Given an AxonOpsPlatform CR with spec.timeSeries.external.hosts configured
