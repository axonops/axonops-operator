Feature: AxonOpsPlatform components
  ...
  I want to configure external access to AxonOpsPlatform components (Dashboard, Server API, Server Agent)
  ...
  Scenario: Enable dashboard Ingress
    Given an AxonOpsPlatform CR with spec.dashboard.ingress.enabled set to true
    ...
    And the Ingress should be owned by the AxonOpsPlatform CR

  Scenario: Enable server agent port Ingress
    Given an AxonOpsPlatform CR with spec.server.agentIngress.enabled set to true
    ...
    And the AxonOpsPlatform status should reflect "AgentIngressReady=True"

  Scenario: Enable server API port Ingress
    Given an AxonOpsPlatform CR with spec.server.apiIngress.enabled set to true
  ...
  Scenario: Disable previously enabled dashboard Ingress
    Given an existing AxonOpsPlatform CR with Dashboard Ingress enabled
    ...
    And the AxonOpsPlatform status should reflect the Ingress removal

  Scenario: Enable Gateway API for dashboard
    Given an AxonOpsPlatform CR with spec.dashboard.gateway.enabled set to true
    ...
    And both resources should be owned by the AxonOpsPlatform CR

  Scenario: Enable Gateway API for server agent
    Given an AxonOpsPlatform CR with spec.server.agentGateway.enabled set to true
    ...
    And the AxonOpsPlatform status should reflect "AgentGatewayReady=True"

  Scenario: Enable Gateway API for server API
    Given an AxonOpsPlatform CR with spec.server.apiGateway.enabled set to true
    ...
    And the AxonOpsPlatform status should reflect "ApiGatewayReady=True"
  ...
  Scenario: Both Ingress and Gateway can be enabled simultaneously
    ...
    And the AxonOpsPlatform status should show both as "Ready=True"

  Scenario: Missing gatewayClassName when Gateway enabled
    Given an AxonOpsPlatform CR with spec.dashboard.gateway.enabled set to true
    And spec.dashboard.gateway.gatewayClassName is omitted or empty
    When the operator reconciles the AxonOpsPlatform CR
    Then the AxonOpsPlatform status should have condition "DashboardGatewayReady=False"
