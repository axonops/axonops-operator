Feature: Manage database authentication credentials lifecycle
  ...
  Scenario: Use existing Secret via secretRef
    ...
    And an AxonOpsServer CR with spec.timeSeries.authentication.secretRef set to "db-creds"
    ...
    And the AxonOpsServer status should have condition "TimeSeriesReady=True"
  ...
  Scenario: Auto-generate credentials when none provided
    Given an AxonOpsServer CR with spec.timeSeries.authentication fully omitted
    When the operator reconciles the AxonOpsServer CR
  ...
  Scenario: Referenced Secret does not exist
    Given an AxonOpsServer CR with spec.timeSeries.authentication.secretRef set to "nonexistent"
    And no Secret named "nonexistent" exists
    When the operator reconciles the AxonOpsServer CR
    Then the AxonOpsServer status should have condition "TimeSeriesReady=False"
  ...
  Scenario: Referenced Secret exists but is missing required key
    ...
    And an AxonOpsServer CR with spec.timeSeries.authentication.secretRef set to "incomplete-creds"
    When the operator reconciles the AxonOpsServer CR
    Then the AxonOpsServer status should have condition "TimeSeriesReady=False"
  ...
  Scenario: Update credentials from SecretRef to inline
    Given an existing AxonOpsServer CR using spec.timeSeries.authentication.secretRef
    ...
    And the AxonOpsServer status should transition to "Ready=True"
