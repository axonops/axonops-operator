Feature: AxonOpsPlatform TLS

  As a cluster operator
  I want the AxonOpsPlatform controller to automatically manage TLS certificates using cert-manager
  So that I don't need to manually provision and rotate TLS certificates

  Background:
    Given the AxonOpsPlatform controller is deployed
  ...
  Scenario: Create Certificate resources and reference default ClusterIssuer
    ...
    When I create an AxonOpsPlatform CR with TLS enabled
    Then the controller should create a Certificate resource for the AxonOpsPlatform TLS
  ...
  Scenario: Create or update default ClusterIssuer for AxonOpsPlatform
    Given cert-manager is installed
    When I create an AxonOpsPlatform CR with TLS enabled
    Then the controller should ensure a ClusterIssuer exists for AxonOpsPlatform TLS
    ...
    And the ClusterIssuer should NOT be deleted when the AxonOpsPlatform CR is deleted (shared resource)

  Scenario: AxonOpsPlatform spec
    Given cert-manager is installed
    And a custom ClusterIssuer named "my-custom-issuer" exists
    When I create an AxonOpsPlatform CR with spec.tls.issuer.name="my-custom-issuer"
  ...
  Scenario: TLS certificate renewal and rotation lifecycle
    Given cert-manager is installed
    And an AxonOpsPlatform CR with TLS enabled exists
    ...
    And the AxonOpsPlatform components should pick up the updated certificate without manual restart

  Scenario: Handle ClusterIssuer creation failure
    Given cert-manager is installed
    But ClusterIssuer creation fails (e.g., due to permission issues)
    When I create an AxonOpsPlatform CR with TLS enabled
    Then the controller should emit an error condition on the AxonOpsPlatform status
    And the AxonOpsPlatform should be in a "FailedToCreateIssuer" or "TLSConfigError" condition
  ...
  Scenario: Certificate resource ownership and garbage collection
    Given cert-manager is installed
    And an AxonOpsPlatform CR exists with TLS enabled
    When I delete the AxonOpsPlatform CR
    Then the Certificate resource should be deleted (owned by AxonOpsPlatform)
    And the ClusterIssuer should NOT be deleted (shared resource, no ownership)
    And the TLS Secret should be deleted (owned by AxonOpsPlatform)
  ...
  Scenario: Support custom CA certificate for ClusterIssuer
    ...
    When I create an AxonOpsPlatform CR with spec.tls.issuer.kind="ClusterIssuer" and spec.tls.issuer.caSecretRef
