Feature: AxonOpsPlatform components
  ...
  Scenario: TimeSeries pods scheduled on nodes matching nodeSelector
    Given an AxonOpsPlatform CR with TimeSeries nodeSelector:
      | key      | value |
      | disktype | ssd   |
  ...
  Scenario: Search pods scheduled on nodes matching nodeSelector
    Given an AxonOpsPlatform CR with Search nodeSelector:
      | key      | value |
      | disktype | ssd   |
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Server pods scheduled on nodes matching nodeSelector
    Given an AxonOpsPlatform CR with Server nodeSelector:
      | key  | value |
      | role | infra |
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Dashboard pods scheduled on nodes matching nodeSelector
    Given an AxonOpsPlatform CR with Dashboard nodeSelector:
      | key  | value    |
      | role | frontend |
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: No nodeSelector when not specified
    Given an AxonOpsPlatform CR with no nodeSelector on any component
    When the operator reconciles the AxonOpsPlatform CR
  ...
  Scenario: Update nodeSelector triggers pod rescheduling
    Given an AxonOpsPlatform CR with TimeSeries nodeSelector "disktype" set to "ssd"
  ...
  Scenario: Remove nodeSelector from component
    Given an AxonOpsPlatform CR with Server nodeSelector "role" set to "infra"
