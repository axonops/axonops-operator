...
func (r *AxonOpsHealthcheckShellReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	log := logf.FromContext(ctx)

	ctx, span := otel.Tracer("axonops-operator").Start(ctx, "reconcile.healthcheckshell", trace.WithAttributes())
	defer func() {
		if err != nil {
	...
	defer func() {
		...
		axonopsmetrics.ReconcileDuration.WithLabelValues("axonopshealthcheckshell", resultStr).Observe(time.Since(start).Seconds())
		axonopsmetrics.ReconcileTotal.WithLabelValues("axonopshealthcheckshell", resultStr).Inc()
	}()
	...
	found := false
	for i, existing := range allHealthchecks.ShellChecks {
		if existing.ID == healthcheck.Status.SyncedHealthcheckID {
			allHealthchecks.ShellChecks[i] = entry
			found = true
			break
		}
	}
	if !found {
		...
		allHealthchecks.ShellChecks = append(allHealthchecks.ShellChecks, entry)
	}
...
func (r *AxonOpsHealthcheckShellReconciler) handleDeletion(ctx context.Context, healthcheck *alertsv1alpha1.AxonOpsHealthcheckShell) (ctrl.Result, error) {
	...
	if healthcheck.Status.SyncedHealthcheckID != "" {
		...
		if err != nil {
			...
			newList := make([]axonops.HealthcheckShell, 0, len(allHealthchecks.ShellChecks))
			for _, h := range allHealthchecks.ShellChecks {
				if h.ID != healthcheck.Status.SyncedHealthcheckID {
					newList = append(newList, h)
				}
			}
			allHealthchecks.ShellChecks = newList
