...
func (r *AxonOpsHealthcheckHTTPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	log := logf.FromContext(ctx)

	ctx, span := otel.Tracer("axonops-operator").Start(ctx, "reconcile.healthcheckhttp", trace.WithAttributes())
	defer func() {
		if err != nil {
	...
	defer func() {
		...
		axonopsmetrics.ReconcileTotal.WithLabelValues("axonopshealthcheckhttp", resultStr).Inc()
	}()
